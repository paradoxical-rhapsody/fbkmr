...
############################################################################################################################
makeVcomps <- function(r, lambda, Z, data.comps, sketch.type, S, indx = NULL) {
  if (sketch.type == 'sub_sampling') Z <- Z[indx,]
############################################################################################################################
  if (is.null(data.comps$knots)) {
    Kpart <- makeKpart(r, Z)
    ############################################################################################################################
    n <- dim(S)[1]; p <- dim(S)[2]
    V <- (diag(1, nrow(Z), nrow(Z)) + lambda[1]*exp(-Kpart))*ifelse(sketch.type == 'sub_sampling',n/p,1) # for sub sampling V = I_p+lambda*Kernel(indx)*n/p
    if (sketch.type == 'gaussian') V <- crossprod(S,V)%*%S
    ############################################################################################################################
...
#' @import utils
kmbayes <- function(y, Z, X = NULL, iter = 1000, family = "gaussian", id = NULL, verbose = TRUE, Znew = NULL, starting.values = NULL, control.params = NULL, varsel = FALSE, groups = NULL, knots = NULL, ztest = NULL, rmethod = "varying", est.h = FALSE, sketch.type = 'None', p = 45, n_subset=1, indx=NULL, Snum = 10, h.post.type = 'fast',est.last.h = 10) {
  ...
  if (!is.null(Znew)) {
    if (class(Znew)[1] != "matrix")  Znew <- as.matrix(Znew)
    stopifnot(is.numeric(Znew), ncol(Znew) == ncol(Z), anyNA(Znew) == FALSE)
  }
  if (!is.null(knots)) {
    if (class(knots)[1] != "matrix")  knots <- as.matrix(knots)
  ...
  if (sketch.type != "None") {
    # Sketching matrix:
    #set.seed(116687)
    new_SI <- sketching(sketch.type,n=nrow(X),p,indx=indx, Snum)
    I <- new_SI$I; S <- new_SI$S; indx <- new_SI$indx
    X <- crossprod(S,X)
    y <- crossprod(S,y)
  }
  ############################################################################################################################
  ...
  if (family == "gaussian") {
    if (is.null(starting.values$beta) | is.null(starting.values$sigsq.eps)) {
      ############################################################################################################################
      lmfit0 <- lm(y_original ~ Z + X_original)
      ############################################################################################################################
    ...
    if (is.null(starting.values$beta) | is.null(starting.values$ystar)) {
      ############################################################################################################################
      probitfit0 <- try(glm(y_original ~ Z + X_original, family = binomial(link = "probit")))
      ############################################################################################################################
  ...
  ## components
  ############################################################################################################################
  Vcomps <- makeVcomps(r = chain$r[1, ], lambda = chain$lambda[1, ], Z = Z, data.comps = data.comps, sketch.type = sketch.type, S = S, indx = indx)
  ############################################################################################################################
  ## start sampling ####
  chain$time1 <- Sys.time()
  for (s in 2:nsamp) {
  # Record time when it's going to start estimating h
  if (s == (iter - est.last.h) + 1) chain$time.start.est.h <- Sys.time()
    ...
    ## beta
    if (!missingX) {
      chain$beta[s,] <- beta.update(X = X, Vinv = Vcomps$Vinv, y = ycont, sigsq.eps = chain$sigsq.eps[s - 1],n_subset=n_subset)
    }

    ## \sigma_\epsilon^2
    if (family == "gaussian") {
      chain$sigsq.eps[s] <- sigsq.eps.update(y = ycont, X = X, beta = chain$beta[s,], Vinv = Vcomps$Vinv, a.eps = control.params$a.sigsq, b.eps = control.params$b.sigsq,n_subset=n_subset)
    }
    ...
    for (comp in 1:data.comps$nlambda) {
      ############################################################################################################################
      varcomps <- lambda.update(r = chain$r[s - 1,], delta = chain$delta[s - 1,], lambda = lambdaSim, whichcomp = comp, y = ycont, X = X, Z = Z, beta = chain$beta[s,], sigsq.eps = chain$sigsq.eps[s], Vcomps = Vcomps, data.comps = data.comps, control.params = control.params, sketch.type=sketch.type, S=S, indx = indx)
      ############################################################################################################################
    ...
    if (length(comp) != 0) {
      if (rmethod == "equal") { ## common r for those variables not being selected
        varcomps <- r.update(r = rSim, whichcomp = comp, delta = chain$delta[s - 1,], lambda = chain$lambda[s,], y = ycont, X = X, beta = chain$beta[s,], sigsq.eps = chain$sigsq.eps[s], Vcomps = Vcomps, Z = Z, data.comps = data.comps, control.params = control.params, rprior.logdens = rprior.logdens, rprop.gen1 = rprop.gen1, rprop.logdens1 = rprop.logdens1, rprop.gen2 = rprop.gen2, rprop.logdens2 = rprop.logdens2, rprop.gen = rprop.gen, rprop.logdens = rprop.logdens,sketch.type=sketch.type, S=S, indx = indx)
        rSim <- varcomps$r
        if (varcomps$acc) {
          Vcomps <- varcomps$Vcomps
        ...
        for (whichr in comp) {
          varcomps <- r.update(r = rSim, whichcomp = whichr, delta = chain$delta[s - 1,], lambda = chain$lambda[s,], y = ycont, X = X, beta = chain$beta[s,], sigsq.eps = chain$sigsq.eps[s], Vcomps = Vcomps, Z = Z, data.comps = data.comps, control.params = control.params, rprior.logdens = rprior.logdens, rprop.gen1 = rprop.gen1, rprop.logdens1 = rprop.logdens1, rprop.gen2 = rprop.gen2, rprop.logdens2 = rprop.logdens2, rprop.gen = rprop.gen, rprop.logdens = rprop.logdens,sketch.type=sketch.type, S=S, indx = indx)
          rSim <- varcomps$r
          if (varcomps$acc) {
            Vcomps <- varcomps$Vcomps
    ...
    if (varsel) {
      ############################################################################################################################
      varcomps <- rdelta.update(r = rSim, delta = chain$delta[s - 1,], lambda = chain$lambda[s,], y = ycont, X = X, beta = chain$beta[s,], sigsq.eps = chain$sigsq.eps[s], Vcomps = Vcomps, Z = Z, ztest = ztest, data.comps = data.comps, control.params = control.params, rprior.logdens = rprior.logdens, rprop.gen1 = rprop.gen1, rprop.logdens1 = rprop.logdens1, rprop.gen2 = rprop.gen2, rprop.logdens2 = rprop.logdens2, rprop.gen = rprop.gen, rprop.logdens = rprop.logdens, sketch.type = sketch.type, S = S, indx = indx)
      ############################################################################################################################
    ...
    if (est.h & s > (iter - est.last.h)) {
      ############################################################################################################################
      if (h.post.type == 'fast') {y_for_h <- y; X_for_h <- X} else if(h.post.type == 'original') {y_for_h <- y_original; X_for_h <- X_original}
      hcomps <- h.update(lambda = chain$lambda[s,], Vcomps = Vcomps, sigsq.eps = chain$sigsq.eps[s], y = y_for_h, X = X_for_h, beta = chain$beta[s,], r = chain$r[s,], Z = Z, data.comps = data.comps, sketch.type = sketch.type, S = S, h.post.type = h.post.type, indx = indx)
      ############################################################################################################################
...
#'
#' @export
summary.bkmrfit <- function(object, q = c(0.025, 0.975), digits = 5, show_ests = TRUE, show_MH = FALSE, ...) {
  x <- object
