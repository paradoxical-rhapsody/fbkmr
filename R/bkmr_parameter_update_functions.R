beta.update <- function(X, Vinv, y, sigsq.eps,n_subset=1) {
  XVinv <- crossprod(X, Vinv)
  XvinvX<-try(chol(XVinv %*% X), silent=TRUE)
  if(class(XvinvX)[1] == "try-error") {
    sigsvd <- svd(XVinv %*% X)
    Vbeta <- t(sigsvd$v %*% (t(sigsvd$u) * 1/(sigsvd$d)))}
  else{Vbeta <- chol2inv(XvinvX)}
  ...
  n01 <- rnorm(ncol(X))
  betahat + crossprod(sqrt(sigsq.eps)*cholVbeta/sqrt(n_subset), n01)
}

sigsq.eps.update <- function(y, X, beta, Vinv, a.eps=1e-3, b.eps=1e-3,n_subset=1) {
  mu <- y - X%*%beta
  prec.y <- rgamma(1, shape=a.eps + nrow(X)/2*n_subset, rate=b.eps + 1/2*crossprod(mu, Vinv)%*%mu*n_subset)
  1/prec.y
}
...
r.update <- function(r, whichcomp, delta, lambda, y, X, beta, sigsq.eps, Vcomps, Z, data.comps, control.params, rprop.gen, rprop.logdens, rprior.logdens, sketch.type, S, indx,...) {
  ...
  ## M-H step
  return(MHstep(r=r, lambda=lambda, lambda.star=lambda.star, r.star=r.star, delta=delta, delta.star=delta.star, y=y, X=X, Z=Z, beta=beta, sigsq.eps=sigsq.eps, diffpriors=diffpriors, negdifflogproposal=negdifflogproposal, Vcomps=Vcomps, move.type=move.type, data.comps=data.comps,sketch.type=sketch.type, S=S, indx = indx))
}
############################################################################################################################
rdelta.comp.update <- function(r, delta, lambda, y, X, beta, sigsq.eps, Vcomps, Z, ztest, data.comps, control.params, rprop.gen2, rprop.logdens1, rprior.logdens, rprior.logdens2, rprop.logdens2, rprop.gen1, sketch.type, S, indx, ...) { ## individual variable selection
  ############################################################################################################################
...
############################################################################################################################
lambda.update <- function(r, delta, lambda, whichcomp=1, y, X, Z = Z, beta, sigsq.eps, Vcomps, data.comps, control.params, sketch.type, S, indx) {
  ############################################################################################################################
...
############################################################################################################################
MHstep <- function(r, lambda, lambda.star, r.star, delta, delta.star, y, X, Z, beta, sigsq.eps, diffpriors, negdifflogproposal, Vcomps, move.type, data.comps, sketch.type, S, indx) {
  ## compute log M-H ratio
  Vcomps.star <- makeVcomps(r.star, lambda.star, Z, data.comps, sketch.type, S, indx = indx)
  ############################################################################################################################
  mu <- y - X%*%beta
...
############################################################################################################################
h.update <- function(lambda, Vcomps, sigsq.eps, y, X, beta, r, Z, data.comps, sketch.type, S, h.post.type = h.post.type, indx) {
  ############################################################################################################################
  if (is.null(Vcomps) | h.post.type == 'original') {
    ############################################################################################################################
    Vcomps <- makeVcomps(r = r, lambda = lambda, Z = Z, data.comps = data.comps, sketch.type = 'None', S, indx = indx)
    ############################################################################################################################
  }
  if(is.null(Vcomps$Q)) {
    if (sketch.type == 'sub_sampling' & h.post.type != 'original') Z <- Z[indx,]
    Kpart <- makeKpart(r, Z)
    ############################################################################################################################
    if (sketch.type == 'None' |  h.post.type == 'original'){K <- exp(-Kpart)
    } else if (sketch.type == 'gaussian' &  h.post.type != 'original'){
      K <- crossprod(S,exp(-Kpart))%*%S
    } else if (sketch.type == 'sub_sampling' &  h.post.type != 'original'){
      n <- dim(S)[1]; p <- dim(S)[2]
      K <- exp(-Kpart)*n/p
    } else {
      stop("if sketch.type is 'None', h.post.type has to be 'original'")
    }
    ############################################################################################################################
    Vinv <- Vcomps$Vinv
    ...
    h.postvar <- sigsq.eps*lamKVinv#crossprod(S,S)%*%
    ############################################################################################################################
    #h.postvar <- sigsq.eps*lambda*crossprod(S,S)%*%(K - lamKVinv%*%K) ##  I DON'T UNDERSTAND WHY IT'S NOT ##h.postvar <- sigsq.eps*lamKVinv
    ############################################################################################################################
    h.postvar.sqrt <- try(chol(h.postvar), silent=TRUE)
    if(class(h.postvar.sqrt)[1] == "try-error") {
    ...
    ############################################################################################################################
    if (sketch.type != 'None' & h.post.type != 'original') {
      hsamp <- S%*%(h.postmean + crossprod(h.postvar.sqrt, rnorm(length(h.postmean)))) # get it back to the original nth dimention
    } else { # the original line of code
      hsamp <- h.postmean + crossprod(h.postvar.sqrt, rnorm(length(h.postmean)))
    }
    ############################################################################################################################
...
newh.update <- function(Z, Znew, Vcomps, lambda, sigsq.eps, r, y, X, beta, data.comps) {
  ...
  if(is.null(data.comps$knots)) {
    ...
    if(class(root.Sigma.hnew)[1] == "try-error") {
