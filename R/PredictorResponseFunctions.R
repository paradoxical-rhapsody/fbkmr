PredictorResponseUnivarVar <- function(whichz = 1, fit, y, Z, X, method = "approx", ngrid = 50, q.fixed = 0.5, sel = NULL, min.plot.dist = Inf, center = TRUE, z.names = colnames(Z), n_subset=1,...) {
...
#' @inheritParams SingVarRiskSummaries_wasp
#'
...
PredictorResponseUnivar <- function(fit, y = NULL, Z = NULL, X = NULL, which.z = NULL, method = "approx", ngrid = 50, q.fixed = 0.5, sel = NULL, min.plot.dist = Inf, center = TRUE, z.names = colnames(Z), n_subset=1, ...) {
  ...
  pred.resp.univar<-foreach(j=1: n_subset)%dopar%{
    ...
    df <- dplyr::data_frame()
    for(i in which.z) {
      res <- PredictorResponseUnivarVar(whichz = i, fit = fit[[j]], y = y, Z = Z, X = X, method = method, ngrid = ngrid, q.fixed = q.fixed, sel = sel, min.plot.dist = min.plot.dist, center = center, z.names = z.names,  n_subset=n_subset,...)
  ...
  for (i in 2:n_subset){
    pred.resp.univar_overall$est<-pred.resp.univar_overall$est+pred.resp.univar[[i]]$est
    pred.resp.univar_overall$se<-pred.resp.univar_overall$se+pred.resp.univar[[i]]$se
  }
  pred.resp.univar_overall$est<-pred.resp.univar_overall$est/n_subset
  pred.resp.univar_overall$se<-pred.resp.univar_overall$se/n_subset
  }
  pred.resp.univar_overall

}



PredictorResponseBivarPair <- function(fit, y, Z, X, whichz1 = 1, whichz2 = 2, whichz3 = NULL, method = "approx", prob = 0.5, q.fixed = 0.5, sel = NULL, ngrid = 50, min.plot.dist = 0.5, center = TRUE, n_subset=1,...) {
...
PredictorResponseBivar <- function(fit, y = NULL, Z = NULL, X = NULL, z.pairs = NULL, method = "approx", ngrid = 50, q.fixed = 0.5, sel = NULL, min.plot.dist = Inf, center = TRUE, z.names = colnames(Z), verbose = TRUE,  n_subset=1,...) {
  ...
  if (inherits(fit[[j]], "bkmrfit")) {
    if (is.null(y)) y <- fit[[j]]$y
    if (is.null(Z)) Z <- fit[[j]]$Z
    if (is.null(X)) X <- fit[[j]]$X
  }
  ...
  for(i in 1:nrow(z.pairs)) {
    ...
    if(compute) {
      ...
      res <- PredictorResponseBivarPair(fit = fit[[j]], y = y, Z = Z, X = X, whichz1 = whichz1, whichz2 = whichz2, method = method, ngrid = ngrid, q.fixed = q.fixed, sel = sel, min.plot.dist = min.plot.dist, center = center, z.names = z.names,n_subset = n_subset, ...)
      df0 <- res
  ...
  if(n_subset>1){##median combination of normal distributions is taking average of their mean and std
    for (i in 2:n_subset){
    pred.resp.bivar_overall$est<-pred.resp.bivar_overall$est+pred.resp.bivar[[i]]$est
    pred.resp.bivar_overall$se<-pred.resp.bivar_overall$se+pred.resp.bivar[[i]]$se
  }
  pred.resp.bivar_overall$est<-pred.resp.bivar_overall$est/n_subset
  pred.resp.bivar_overall$se<-pred.resp.bivar_overall$se/n_subset
  }
  pred.resp.bivar_overall
}
